...
// ====================================================
  ...
  if (estadoFade > 0) {
    push();
    noStroke();
    fill(255, fadeAlfa); // Rectángulo blanco con opacidad variable
    rect(0, 0, width, height);
    pop();

    if (estadoFade === 1) {
      // ETAPA 1: Yendo a blanco
      fadeAlfa += velocidadFade;
      if (fadeAlfa >= 255) {
        fadeAlfa = 255;
        resetear();      // ¡Hacemos el reseteo duro mientras nadie ve!
        estadoFade = 2;  // Pasamos a la etapa de volver
      }
    } else if (estadoFade === 2) {
      // ETAPA 2: Volviendo de blanco a transparente
      fadeAlfa -= velocidadFade;
      if (fadeAlfa <= 0) {
        fadeAlfa = 0;
        estadoFade = 0;  // Terminó la transición, volvemos a la normalidad
      }
    }
  }
}
